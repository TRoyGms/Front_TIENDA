...
function ServiceForm({ service, onSuccess }) {
  ...
  useEffect(() => {
    console.log("Servicio recibido en el formulario:", service);
    if (service) {
  ...
  const url = service
    ? `${import.meta.env.VITE_API_SERVICES}${service.id}`
    : import.meta.env.VITE_API_SERVICES;
      ...
      <div className="flex flex-col items-center justify-center w-[100%] h-[100%] p-6 rounded-lg">
        ...
        >
          ...
          <div className="w-full py-2 bg-[#4AB0C2] text-white rounded-lg shadow-lg hover:bg-[#38818F] transition duration-300">
            <Button
              name={service ? "Actualizar Servicio" : "Guardar Servicio"}
              onClick={handleSave}/>
          </div>
        </form>
      </div>
